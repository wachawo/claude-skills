...
For async use, apply `@beta_async_tool` to `async def` functions.

**Key tool runner benefits:**

- No manual loop — the SDK calls tools and returns results
- Type-safe tool inputs via decorators
- Tool schemas generated automatically from function signatures
...
**Beta.** Converts [MCP (Model Context Protocol)](https://modelcontextprotocol.io/) tools, prompts, and resources into Anthropic API types for use with the tool runner. Requires `pip install anthropic[mcp]` (Python 3.10+).
...
### MCP Tools with the Tool Runner
...
For synchronous use, apply `mcp_tool` instead of `async_mcp_tool`.
...
The conversion functions raise `UnsupportedMCPValueError` when an MCP value cannot be converted (e.g., unsupported content types like audio, unsupported MIME types).
...
### JSON Output (Pydantic — recommended)
