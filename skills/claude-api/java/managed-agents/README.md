...
> **Bindings not shown here:** This README covers the most common managed-agents flows for Java. If you need a class, method, namespace, field, or behavior not shown here, WebFetch the Java SDK repository **or the corresponding documentation page** from `shared/live-sources.md` rather than guessing. Do not extrapolate from cURL forms or another language's SDK.
...
> ⚠️ **There is no inline agent configuration.** Model, system, and tools live on the agent object, not on the session. Always start with `client.beta().agents().create()` — the session accepts either `.agent(agent.id())` or the typed `BetaManagedAgentsAgentParams.builder()...build()`.
...
When reconnecting mid-session, first list past events for deduplication, then read live events. The cross-variant `id` field is read from the raw `_json()` value:
...
> ℹ️ The Java managed-agents bindings for `user.custom_tool_result` are not yet documented in this skill or in the source app examples. See `shared/managed-agents-events.md` for the wire format and the `anthropic-java` repository for the corresponding param types.
...
// List session resources — entries represent a discriminated union
...
> ℹ️ Listing and downloading files the agent wrote during the session is not yet documented for Java in this skill or in the source app examples. See `shared/managed-agents-events.md` and the `anthropic-java` repository for file list/download bindings.
