...
This file contains WebFetch URLs that retrieve up-to-date information from platform.claude.com and the Agent SDK repositories. Use them when the user needs the latest data, which may have changed since the cached content was captured.

## When to Use WebFetch

- The user explicitly asks for "latest" or "current" information
- Cached data appears to be incorrect
- The user asks about features not covered by the cache
- The user needs specific API details or examples
...
Use these when a managed-agents binding, behavior, or wire-level detail isn't covered by the cached conceptual files `shared/managed-agents-*.md` or `{lang}/managed-agents/README.md`.
...
The `ant` CLI provides terminal access to the Claude API. Every API resource is available as a subcommand. It's a convenient way to create agents, environments, sessions, and other resources from version-controlled YAML and to interactively inspect responses.
...
WebFetch these when a binding (class, method, namespace, field) is not covered in the cached `{lang}/` files or in the managed-agents documentation above. The SDKs include beta managed-agents support for `/v1/agents`, `/v1/sessions`, `/v1/environments`, and related resources — search the repository for `BetaManagedAgents`, `beta.agents`, `beta.sessions`, or the equivalent namespace for that language.
...
1. Use cached content from the language files (note the cache date)
2. Tell the user the data may be out of date
3. Suggest checking platform.claude.com or the GitHub repositories directly
