...
**Use only the exact model IDs listed in this file.** Never guess or construct model IDs — incorrect IDs will produce API errors. Use aliases wherever they're available. For up-to-date information, WebFetch the Models Overview URL from `shared/live-sources.md` or query the Models API directly (see Programmatic Model Discovery below).
...
### Model Descriptions
- **Claude Opus 4.7** — Claude's most powerful model to date — highly autonomous, strong on long-horizon agentic work, knowledge work, vision, and memory. Adaptive thinking only; sampling parameters and `budget_tokens` are removed. 1M context window at standard API pricing (no long-context premium) — see `shared/model-migration.md` → Migrating to Opus 4.7 for breaking changes.
- **Claude Opus 4.6** — the prior-generation Opus. Supports adaptive thinking (recommended), 128K max output tokens (streaming required for large outputs). 1M context window.
...
- **Claude Haiku 4.5** — the fastest and most cost-efficient model for simple tasks.
...
When the user asks for a model by name, use this table to find the correct model ID:
