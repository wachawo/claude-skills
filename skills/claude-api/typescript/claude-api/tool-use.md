...
For the conceptual overview (tool definitions, tool choice, tips), see [shared/tool-use-concepts.md](../../shared/tool-use-concepts.md).
...
**Key tool runner benefits:**

- No manual loop — the SDK calls tools and returns results
- Type-safe tool inputs via Zod schemas
- Tool schemas generated automatically from Zod definitions
...
Use `client.messages.stream()` + `finalMessage()` instead of `.create()` when you need streaming inside a manual loop. Each iteration streams text deltas; `finalMessage()` assembles the full `Message` so you can inspect `stop_reason` and extract tool-use blocks:
...
> **Important:** don't wrap `.on()` events in `new Promise()` to collect the final message — use `stream.finalMessage()`. The SDK handles all error/abort/completion states itself.

> **Error handling in the loop:** use the SDK's typed exceptions (e.g., `Anthropic.RateLimitError`, `Anthropic.APIError`) — see examples in the [Error Handling](./README.md#error-handling) section. Do not check error messages through string comparison.

> **SDK types:** for all API-related data structures, use `Anthropic.MessageParam`, `Anthropic.Tool`, `Anthropic.ToolUseBlock`, `Anthropic.ToolResultBlockParam`, `Anthropic.Message`, etc. Do not redefine equivalent interfaces.
...
`type` has a version suffix; `name` is fixed for each interface. Pass plain object literals — the `ToolUnion` type is satisfied structurally. **The `name`/`type` pair must match the interface**: mixing `str_replace_based_edit_tool` (the 20250728 name) with `text_editor_20250124` (which expects `str_replace_editor`) yields TS2322.

**Don't annotate as `Tool[]`** — `Tool` is only the custom-tool variant. Let structural typing infer from the `tools` parameter, or annotate as `Anthropic.Messages.ToolUnion[]` when forced:
...
**Don't mix beta and non-beta types**: if you call `client.beta.messages.create()`, the response `content` is typed as `BetaContentBlock[]` — you can't pass it into non-beta `ContentBlockParam[]` without narrowing each element.
...
`__dirname` is not available in ES modules. For paths relative to the script, use `import.meta.url`:
...
Or use a path relative to CWD if the script runs from a known directory: `readFileSync("./sample.pdf")`.
...
Full implementation examples via WebFetch:
