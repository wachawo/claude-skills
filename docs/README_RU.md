...
|---|---|
| `↑` / `↓` | Перемещение курсора |
| `←` / `→` | Свернуть / развернуть скилл |
| `Shift+↑` / `Shift+↓` | Прокрутка панели diff'а |
...
Снять галку с уже установленного файла — он будет помечен на удаление при
Apply (отовсюду, где найден). Пустые директории скиллов чистятся.
